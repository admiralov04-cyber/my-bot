...
def get_user(uid, name=None):
    ...
    uid = str(uid)
    if uid not in DB:
        DB[uid] = {"name": name or "Игрок", "balance": 10000, "last_daily": None, "current_game": None, "business": False, "vip": False, "cards": 1, "cards_income": CARD_BASE_INCOME, "cards_price": CARD_BASE_PRICE, "tax_balance": 0, "tax_limit": 5000000, "mining_start": None, "mined_total": 0, "reg_date": str(datetime.date.today()), "earned": 0, "lost": 0, "games": 0, "cases_opened": 0}
        save_db()
    return DB[uid]
...
def get_mining_info(user):
    ...
    return {"cards": cards, "income_per_card": inc, "total_income": cards * inc, "next_price": user.get("cards_price", CARD_BASE_PRICE), "tax": user.get("tax_balance", 0), "tax_limit": user.get("tax_limit", 5000000)}
...
async def buttons(update, context):
    ...
    if d == "profile":
        ...
        user = get_user(uid)
        info = get_mining_info(user)
        txt = f"👤 Профиль\n\n💰 Баланс: {user['balance']:,} 🪙\n📅 Регистрация: {user['reg_date']}\n🎮 Игр: {user['games']}\n🎁 Кейсов: {user['cases_opened']}\n\n⛏ Майнинг:\n🖥 Видеокарт: {info['cards']} шт.\n💷 Доход: {info['total_income']:,} 🪙/час\n💎 Намайнено: {user['mined_total']:,} 🪙\n💸 Налог: {user['tax_balance']:,}/{user['tax_limit']:,} 🪙\n🏪 Бизнес: {'✅' if user['business'] else '❌'}\n💎 VIP: {'✅' if user['vip'] else '❌'}\n\n💚 Заработано: {user['earned']:,} 🪙\n💔 Проиграно: {user['lost']:,} 🪙"
        kb = [[InlineKeyboardButton("⛏ Собрать", callback_data="collect")], [InlineKeyboardButton("💰 Налог", callback_data="paytax")], [InlineKeyboardButton("↩️ Назад", callback_data="menu")]]
        ...
        kb = [[InlineKeyboardButton("🪙 Монетка", callback_data="game_coin")], [InlineKeyboardButton("🎲 Кости", callback_data="game_dice")], [InlineKeyboardButton("🎰 Слоты", callback_data="game_slots")], [InlineKeyboardButton("↩️ Назад", callback_data="menu")]]
        ...
        if user.get("mining_start"):
            try:
                ...
                pass
        txt = f"⛏ Майнинг ферма\n\n💷 Доход: {info['total_income']:,} 🪙/час\n📝 Видеокарты: {info['cards']} шт./♾️ шт.\n💵 Доход с карты: {info['income_per_card']:,} 🪙/час\n🆙 Цена следующей: {info['next_price']:,} 🪙\n\n💸 Налоги: {info['tax']:,}/{info['tax_limit']:,} 🪙\n💰 На счету: {user['balance']:,} 🪙"
        if pending > 0:
            txt += f"\n⏳ Ожидает: {pending:,} 🪙"
        ...
        kb = [[InlineKeyboardButton("🏪 Бизнес", callback_data="shop_business")], [InlineKeyboardButton("💎 VIP", callback_data="shop_vip")], [InlineKeyboardButton("↩️ Назад", callback_data="menu")]]
